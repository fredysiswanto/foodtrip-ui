...
import { useRestoDishDetail } from '..';
import { Button, Card, VStack, Spinner, Alert } from '@foodtrip/ui';
import { CreateDishInputType } from '@foodtrip/types';
import { RestoDishForm } from '../components/RestoDishForm';
import { useRestoUpdateDish } from '../hooks/useRestoDish';

export function RestoDishEditPage() {
  ...
  return (
    <VStack gap="lg">
      <div>
        <Button
          variant="ghost"
          onClick={() => navigate(`/restaurant-admin/dishes/${id}`)}
        >
          ← Back to Dish
        </Button>
